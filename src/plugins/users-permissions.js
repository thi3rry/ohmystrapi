 ...
 */
export const useUsersPermissionsApi = (strapi) => {
    const contentManager = useContentManagerApi(strapi);
    return {
        /**
        ...
        loginRequest({identifier, password}) {
            return (strapi.createFetch({withAuthHeaders: false, returnResponseData: true})(
                `/auth/local`,
                {
                    body: {
                        identifier,
        ...
        async forgotPasswordRequest({email}) {
            return await (strapi.createFetch({withAuthHeaders: false})(
                `${strapi.apiBaseUrl}/auth/forgot-password`,
                {
                    method: 'POST',
                    body: {email}
                }
        ...
        async changeForgottenPasswordRequest ({code, password, passwordConfirmation}) {
            return await (strapi.createFetch({withAuthHeaders: false})(
                `${strapi.apiBaseUrl}/auth/reset-password`,
                {
                    method: 'POST',
                    body: {
                        code,
        ...
        async changePassword({currentPassword, newPassword, newPasswordConfirmation}) {
            const data = await (strapi.createFetch()(
                `${strapi.apiBaseUrl}/auth/change-password`,
                {
                    method: 'POST',
                    body: {
        ...
        async registerRequest(registrationDetails) {
            return await (strapi.createFetch({withAuthHeaders: false})(`${strapi.apiBaseUrl}/auth/local/register`, {method: 'POST', body: registrationDetails}));
        },
         ...
         */
        async me() {
            return await (strapi.createFetch()(`${strapi.apiBaseUrl}/users/me`));
        },
        updateMe({id, ...params}) {
            return contentManager.update('users', id, params);
        },
    };
};
